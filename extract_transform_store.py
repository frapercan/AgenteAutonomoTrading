...
def dataset_sliding_window_supervised_val(dataset,close_values,horizon, forecast, output):
    num_features = dataset.shape[1]
    num_samples = dataset.shape[0] - horizon - forecast
    X = np.zeros((num_samples, horizon, dataset.shape[1]))
    Y = np.zeros((num_samples))
    CLOSE = np.zeros((num_samples))
    for i in range(num_samples):
        subset = np.array(dataset[i:i + horizon, :num_features])
        X[i, :, :] = subset
        log_change = np.sum(np.array(dataset[i + horizon:i + horizon + forecast, output]))
...
class ETS:
    ...
    def generate_validation_set(self):
        ...
        query = \
            'from(bucket: "{pair}") \
          |> range(start: -22d, stop: -13d)\
          |> filter(fn: (r) => r._measurement == "{pair}") \
          |> filter(fn: (r) => r["_field"] == "close_return" or r["_field"] == "high_return" or r["_field"] == "low_return" or r["_field"] == "open_return" or r["_field"] == "volume_return" or r["_field"] == "close")\
          |> yield(name: "raw") \
        ...
        close_values = values[close_index]
        values = np.delete(values,close_index,axis = 0)
        del columns[close_index]
        scale = MinMaxScaler((-1,1))
        feature_range = np.vstack(([ self.features_range['max'+column] for column in columns],[ -self.features_range['max'+column] for column in columns]))

        scale.fit(feature_range)
        values = np.vstack([value.values for value in values])
        values = scale.transform(values.transpose())
        volume_feature = columns.index("volume_return")
        values[:,volume_feature] = values[:,volume_feature]/10

        output_feature = columns.index(self.output_feature)
        X,Y,CLOSE = dataset_sliding_window_supervised_val(values, close_values ,self.horizon,self.forecast,output_feature)

        np.savez_compressed(os.path.join("./validation_data/")+"valset", X=X, Y=Y, CLOSE=CLOSE)
    ...
    def get_minimum_values(self):
        ...
        query_min = \
            'from(bucket: "{pair}") \
          |> range(start: -5000d, stop: -{test_set}d)\
          |> filter(fn: (r) => r._measurement == "{pair}") \
