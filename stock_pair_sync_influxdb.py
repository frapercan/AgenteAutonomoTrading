    ...
    def serialize_points(self, response):
        ...
        for tick in list(response):
            if lag_response:
                ...
                open_transform = np.log(tick[1] / open_price)
                low_transform = np.log(tick[2] / low_price)
                high_transform = np.log(tick[3] / high_price)
                close_transform = np.log(tick[4] / close_price)
                volume_transform = np.log(tick[5] / volume)
                open_price = float(tick[1])
                low_price = float(tick[2])
                high_price = float(tick[3])
                close_price = float(tick[4])
                ...
                point = Point(self.pair).field('close', close_price).field('open', open_price).field('high', high_price) \
                    ...
                    .field('volume', volume)\
                    .time(timestamp)\
                    .field('open_transform', open_transform)\
                    .field('low_transform', low_transform)\
                    .field('high_transform', high_transform)\
                    .field('close_transform', close_transform)\
                    .field('volume_transform', volume_transform)
                points.append(point)
